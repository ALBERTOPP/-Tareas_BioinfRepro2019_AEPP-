...
# ==== ========== ==== == ======== ======
Data.Raw  <- read.delim("intento.txt")
signal   <- grep("CDR0", colnames(Data.Raw))  # vector of columns with data
detection <- grep("Detection.Pval", colnames(Data.Raw)) # vector of columns with p-values

# Read probe annotations
# ==== ===== ===========
# Importing Annotation Dataset 
annot <- read.delim("MouseRef-8_annot_full.txt")
# Select only rows obtained in Data.raw sampling
annot <- annot %>% dplyr::filter(ProbeID %in% Data.Raw$ID_REF)
